...
void Widget::mousePressEvent(QMouseEvent * e) //鼠标按下事件
{
    ...
    for (int i = 4; i >=0 ; --i)
    {
        ...
        QPoint nw_pt=pt+rule.po[i];//某一方面的
        //qDebug()<<"nw_pt="<<nw_pt.rx()<<","<<nw_pt.ry();
        Chess temp02(nw_pt,!m_isBlackTurn);//相反颜色

        if(m_Chess.contains(temp02)||i==4)
        {
            qDebug()<<"i="<<i;

            if(i==4)
                rule.calcuate(nw_pt,m_isBlackTurn,i);
            ...
            qDebug()<<"rule.SighMax["<<i<<"]="<<rule.SighMax[i];
