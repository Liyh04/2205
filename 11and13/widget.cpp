...
Widget::Widget(QWidget *parent) : QWidget(parent) , ui(new Ui::Widget)//初始化ui界面
{
    ...
    setStyleSheet("background-color: rgb(150, 217, 245);");
    ui->setupUi(this);
    //设置窗口
    this->init();
...
{
    for(int i1=0;i1<n_row;i1++)for(int j1=0;j1<n_row;j1++)if_scanned[i1][j1]=0;
}
